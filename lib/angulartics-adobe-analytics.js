      ...
      $analyticsProvider.registerEventTrack(function (eventType, properties) {
        ...
        if (window.s && eventType) {
          this.$inject(['$interval', function ($interval) {
            ...
            // Shorthand syntax for tracking single contextData keys
            if (props.context) {
            ...
            // For tracking multiple contextData keys at once
            } else if (props.contextData) {
              angular.forEach(props.contextData, function (key, val) {
                linkTrackVars.push('contextData.' + val)
              })
            }
